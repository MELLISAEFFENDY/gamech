...
Automation:Section('Timing Settings')
...
Automation:Slider('Auto Cast Delay (s)', {location = flags, flag = 'autocastdelay', min = 0.1, max = 10, default = 2})
Automation:Toggle('Random Cast Power', {location = flags, flag = 'randomcast', default = false})
Automation:Slider('Min Cast Power %', {location = flags, flag = 'mincastpower', min = 50, max = 100, default = 85})
Automation:Slider('Max Cast Power %', {location = flags, flag = 'maxcastpower', min = 50, max = 100, default = 100})
...
Automation:Slider('Shake Delay Range (ms)', {location = flags, flag = 'shakedelayrange', min = 0, max = 500, default = 150})
Automation:Toggle('Auto Reel', {location = flags, flag = 'autoreel'})
Automation:Slider('Auto Reel Delay (s)', {location = flags, flag = 'autoreeldelay', min = 0.1, max = 5, default = 0.5})
Automation:Toggle('Random Reel Power', {location = flags, flag = 'randomreel', default = false})
Automation:Slider('Min Reel Power %', {location = flags, flag = 'minreelpower', min = 70, max = 100, default = 90})
Automation:Slider('Max Reel Power %', {location = flags, flag = 'maxreelpower', min = 70, max = 100, default = 100})
-----
...
if CheckFunc(hookmetamethod) then
    local old; old = hookmetamethod(game, "__namecall", function(self, ...)
        ...
        elseif method == 'FireServer' and self.Name == 'cast' and flags['perfectcast'] then
            -- Randomize perfect cast to avoid detection
            if flags['stealthmode'] and flags['humanbehavior'] > 0 then
                if math.random(1, 100) <= flags['humanbehavior'] then
                    args[1] = math.random(85, 99) -- Imperfect cast sometimes
                else
                    args[1] = 100
                end
            else

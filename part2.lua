...
-- Default min/max delay (can be changed via UI)
flags['cast_delay_min'] = flags['cast_delay_min'] or 0.6
flags['cast_delay_max'] = flags['cast_delay_max'] or 1.2
flags['reel_delay_min'] = flags['reel_delay_min'] or 0.6
flags['reel_delay_max'] = flags['reel_delay_max'] or 1.2
flags['shake_delay_min'] = flags['shake_delay_min'] or 0.2
flags['shake_delay_max'] = flags['shake_delay_max'] or 0.6
...
Automation:Slider('Cast Delay Min', {location = flags, flag = 'cast_delay_min', min = 0.3, max = 2, precise = true})
Automation:Slider('Cast Delay Max', {location = flags, flag = 'cast_delay_max', min = 0.4, max = 3, precise = true})
Automation:Toggle('Auto Shake', {location = flags, flag = 'autoshake'})
Automation:Slider('Shake Delay Min', {location = flags, flag = 'shake_delay_min', min = 0.1, max = 1.5, precise = true})
Automation:Slider('Shake Delay Max', {location = flags, flag = 'shake_delay_max', min = 0.2, max = 2, precise = true})
Automation:Toggle('Auto Reel', {location = flags, flag = 'autoreel'})
Automation:Slider('Reel Delay Min', {location = flags, flag = 'reel_delay_min', min = 0.3, max = 2, precise = true})
Automation:Slider('Reel Delay Max', {location = flags, flag = 'reel_delay_max', min = 0.4, max = 3, precise = true})
-----
...
state.connections['heartbeat'] = RunService.Heartbeat:Connect(function()
    ...
    safe(function()
        ...
        if flags['autoshake'] then
            safe(function()
                local now = os.clock()
                local minDelay = tonumber(flags['shake_delay_min']) or 0.2
                local maxDelay = tonumber(flags['shake_delay_max']) or 0.6
                if minDelay > maxDelay then minDelay, maxDelay = maxDelay, minDelay end
                ...
                if shake then
                    ...
                    if sz then
                        ...
                        if btn and (now - state.lastShakeTime) >= state.nextShakeDelay then
                            state.lastShakeTime = now
                            state.nextShakeDelay = math.random() * (maxDelay - minDelay) + minDelay
        ...
        if flags['autocast'] then
            safe(function()
                local now = os.clock()
                local rod = FindRod()
                local minDelay = tonumber(flags['cast_delay_min']) or 0.6
                local maxDelay = tonumber(flags['cast_delay_max']) or 1.2
                if minDelay > maxDelay then minDelay, maxDelay = maxDelay, minDelay end
                if rod and rod:FindFirstChild('values') and rod.values:FindFirstChild('lure') then
                    if rod.values.lure.Value <= .001 and (now - state.lastCastTime) >= state.nextCastDelay then
                        state.lastCastTime = now
                        state.nextCastDelay = math.random() * (maxDelay - minDelay) + minDelay
        ...
        if flags['autoreel'] then
            safe(function()
                local now = os.clock()
                local rod = FindRod()
                local minDelay = tonumber(flags['reel_delay_min']) or 0.6
                local maxDelay = tonumber(flags['reel_delay_max']) or 1.2
                if minDelay > maxDelay then minDelay, maxDelay = maxDelay, minDelay end
                if rod and rod:FindFirstChild('values') and rod.values:FindFirstChild('lure') then
                    if rod.values.lure.Value == PERFECT_VALUE and (now - state.lastReelTime) >= state.nextReelDelay then
                        state.lastReelTime = now
                        state.nextReelDelay = math.random() * (maxDelay - minDelay) + minDelay

...
local TeleportLocations = {
    ['Zones'] = {
        ...
        ["Anlantis"] = CFrame.new(-4244.42, -603.41, 1826.31),
        ["Harvest Spike"] = CFrame.new(-1326.22, 139.59, 1536.08),
        ["Vertigo"] = CFrame.new(-108.32, -732.51, 1210.94),
        ["ALTAR"] = CFrame.new(1315.43, -802.31, -87.96)
    },
    ['Rods'] = {
        ...
        if flags['autocast'] then
            pcall(function()
                ...
                if rod ~= nil and rod['values']['lure'].Value <= .001 then
                    if flags['stealthmode'] then
                        -- Random delay before casting (1-3 seconds)
                        task.wait(randomDelay(1, 3))
                        
                        -- Sometimes use imperfect cast power
                        local castPower = 100
                        if flags['humanbehavior'] > 0 and math.random(1, 100) <= flags['humanbehavior'] then
            ...
            pcall(function()
                ...
                if rod ~= nil and rod['values']['lure'].Value == 100 then
                    if flags['stealthmode'] then
                        ...
                        -- Sometimes fail the reel (human behavior)
                        local reelSuccess = true
                        local reelPower = 100
                        
                        if flags['humanbehavior'] > 0 and math.random(1, 100) <= (flags['humanbehavior'] / 3) then
                            reelSuccess = false
